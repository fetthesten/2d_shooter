...
func _ready():
	if current_camera == null:
		current_camera = Main.current_scene.get_node('Camera2D')
		current_camera.current = true
	print('** ',Main.current_scene.name,' loaded **')
	...
	for n in Main.current_scene.get_children():
		if n is TileMap:
